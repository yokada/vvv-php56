...
nginx_setup() {
  ...
  if [[ ! -e /etc/nginx/server.crt ]]; then
	  ...
	  vvvsigncert="$(openssl req -new -x509 \
            ...
            -days 3650 \
            -subj /CN=*.wordpress-develop.dev/CN=*.wordpress.dev/CN=*.vvv2.dev 2>&1)"
	  echo "$vvvsigncert"
  fi
...
phpfpm_setup() {
  ...
  cp "/srv/config/php-config/php56-fpm.conf" "/etc/php/5.6/fpm/php-fpm.conf"
  ...
  echo " * Copied /srv/config/php-config/php56-fpm.conf     to /etc/php/5.6/fpm/php-fpm.conf"
...
mailcatcher_setup() {
  ...
  if [[ -f "/etc/php/5.6/mods-available/mailcatcher.ini" ]]; then
    echo " *" Mailcatcher php5.6 fpm already configured.
  else
...
PHP
    ...
    noroot wp core install --url=local2.wordpress.dev --quiet --title="Local WordPress Dev" --admin_name=admin --admin_email="admin@local.dev" --admin_password="password"
  else
...
wordpress_develop(){
  ...
  if [[ ! -d "/srv/www/wordpress-develop" ]]; then
    ...
    echo "Installing local npm packages for src2.wordpress-develop.dev, this may take several minutes."
    noroot npm install

    echo "Initializing grunt and creating build2.wordpress-develop.dev, this may take several minutes."
    noroot grunt
    ...
    echo "Creating wp-config.php for src2.wordpress-develop.dev and build2.wordpress-develop.dev."
...
define( 'WP_HOME', 'http://build2.wordpress-develop.dev' );
define( 'WP_SITEURL', 'http://build2.wordpress-develop.dev' );
}

define( 'WP_DEBUG', true );
PHP
    echo "Installing src2.wordpress-develop.dev."
    noroot wp core install --url=src2.wordpress-develop.dev --quiet --title="WordPress Develop" --admin_name=admin --admin_email="admin@local.dev" --admin_password="password"
...
echo "For further setup instructions, visit http://vvv2.dev"
